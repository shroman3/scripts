...
dsToAlgs = { "abtree" :         ["rwlock", "htm_rwlock", "lockfree"],
             ...
             "lflist" :         ["rwlock", "htm_rwlock", "lockfree", "timnat"],
             "lazylist" :       ["rwlock", "htm_rwlock", "lockfree", "rlu"],
             "citrus" :         ["rwlock", "htm_rwlock", "lockfree", "rlu"],
             "skiplistlock" :   ["rwlock", "htm_rwlock", "lockfree", "timnat"]
            }
...
allalgs =[ "rwlock", "htm_rwlock", "lockfree", "timnat", "rlu", "unsafe"]
...
def plot_bar(codec,work_th,rq_th,u,rq,k,rqsize):
    ...
    title = dsToName[codec]
    if not clean:
        title += "\n" + " ".join(state)
    df = pd.read_sql(query, conn)
    df = df.set_index('x')
    df = df.reindex(index=dsToAlgs[codec])
    plt.figure()
    ...
    filename = "bar-"+metric_name+"-"+codec+"-"
    filename += "-".join(state)
