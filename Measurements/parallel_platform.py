...
#			 run_command("java -Xms15g -Xmx15g -jar client.jar " + self.exp + " " 
#						 + self.codec + " " + self.k + " " + self.r + " " + self.z + " " 
#						 + self.random_name + " " + str(self.servers_num) + " " + self.step_size, True)
			call(["java", "-d64", "-server", "-Xms15g", "-Xmx15g", "-jar", "client.jar", self.exp, self.codec, self.k, self.r, self.z, self.random_name, str(self.servers_num), self.step_size])
...
#			 csvwriter.writerow(csvrow)
	def parse_network_logs(self, name, extension):
		...
		csvwriter = csv.writer(output)
		csvwriter.writerow(['exp', 'codec', 'random', 'k', 'r', 'z', 'servers_num', 'time(ms)', 'tag', 'size', 'server_id', 'log_type', 'machine', 'disk', 'process_id'])
